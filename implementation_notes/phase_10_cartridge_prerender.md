...
- **POC landed for GB.** `core/cartridge_render.py` (compositor + disk cache) and the grid path that shows one flat `Gtk.Picture`; `assets/images/cartridges/GB.svg` is the first SVG frame. `gir1.2-rsvg-2.0` added to `make install-sys-deps`, the `.deb` deps and the AppImage recipe.
- Consoles with only a PNG frame (FC/SFC/GBA) still use the old runtime overlay: `CARTRIDGE_COVER_FRAMES`/`CARTRIDGE_ITEM_SIZES` and the `Gtk.Fixed`/`Gtk.Overlay` wiring stay until those three are re-authored as SVGs, which is when the cleanup in *Migration & cleanup* happens.
- Not yet done: eager pre-render during cover sync, a "rebuild cartridge cache" maintenance action, and validating the composite on a HiDPI display (it renders at 2× and is handed to GTK as a `Gdk.Texture`).
